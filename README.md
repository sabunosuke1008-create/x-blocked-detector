...
**予算モード**(巨大アカウント向け・全ソース横断):
`--max-pages N` 収集ページ総量の上限 / `--time-budget SEC` 収集フェーズの時間上限。
タイムライン系は新しい順に流れるため、上限到達時は自動的に「直近の接点」で打ち切りとなる。検知した被ブロックは即時 `  [!!] @user ...` 行で逐次表示される(スキャン完了を待たない)。
...
| `max_pages` / `time_budget_seconds` | 収集ページ総量/収集時間の横断予算(既定0=無制限) |
